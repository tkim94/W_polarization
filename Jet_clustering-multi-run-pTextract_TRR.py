...
for entry in range(init, fin):
    ...
    if 25 in pids:
        ...
        #for pin in range(0, 20):
            ...
            # For pp->h->ww, w+->j j , w- -> e ve~ only look for w+
            if (part.PID == 24 and part.Status == 22):
    ...
    # No higgs event
        ...
        #for pin in range(0, 20):
            ...
            #print (part.PID, part.Status) # Debug
            if(abs(part.PID) == 24 and part.Status == 22):
                wpart.SetPtEtaPhiE(part.PT, part.Eta, part.Phi, part.E) # H-> WW need to be fixed (5/17/2019) # Fixed?
                #print "Got wpart"
                #if part.PT > 100:
                #    print part.PT
                #print part.PT
            #print("Mass = "+str(np.sqrt(wpart.E()**2-wpart.Px()**2-wpart.Py()**2-wpart.Pz()**2)))
